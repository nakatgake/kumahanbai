...
def calculate_spray_price(product, case_count: int, rank: models.CustomerRank) -> int:
    ...
    # 価格が設定されていない(0)の場合はランク価格を適用
    if not price or price <= 0:
        price = get_price_for_rank(product, rank)
    
    return int(price)

def get_price_for_rank(product, rank: models.CustomerRank) -> int:
    """顧客ランクに応じた標準価格を取得"""
    rank_price_map = {
...
def generate_invoice_pdf_content(invoice: models.Invoice):
    ...
    try:
        ...
        for order in invoice.orders:
            ...
            if order.quotation:
                for item in order.quotation.items:
                    # 品名の長さから必要な行の高さを計算 (fpdf2のmulti_cellをドライラン)
                    line_height = 7
                    desc_width = 90
                    # テキストを行に分割
                    lines = pdf.multi_cell(desc_width, line_height, item.description, dry_run=True, output="LINES")
                    h = max(line_height, len(lines) * line_height)

                    # 改ページチェック
                    if pdf.get_y() + h > page_bottom_limit:
                        pdf.add_page()
                        draw_table_header(pdf)
                    ...
                    # 各列を描画 (MultiCellに合わせて高さを揃える)
                    pdf.cell(10, h, str(row_idx), border=1, align="C")
                    
                    # 品名列 (MultiCell)
                    pdf.set_xy(curr_x + 10, curr_y)
                    pdf.multi_cell(desc_width, line_height, item.description, border=1, align="L")
                    
                    # 残りの列を右側に配置
                    pdf.set_xy(curr_x + 100, curr_y)
                    pdf.cell(20, h, f"{item.quantity}", border=1, align="R")
                    pdf.cell(35, h, f"{int(item.unit_price):,.0f}", border=1, align="R")
                    pdf.cell(35, h, f"{int(item.subtotal):,.0f}", border=1, align="R")
                    
                    # 次の行へ移動
                    pdf.set_xy(curr_x, curr_y + h)
                    row_idx += 1
...
async def agency_create_order(
    ...
    try:
        ...
        # 設定から定数を取得
        TAX_RATE = float(get_system_setting(db, "tax_rate", "0.1"))
        MIN_ORDER = int(get_system_setting(db, "agency_min_order_amount", "10000"))
        ...
        shipping_fee = 0
        if product_total_tax_incl < SHIPPING_THRESHOLD:
...
async def cleanup_unpaid_invoices_execute(
    ...
    try:
        ...
        # 1. 100%安全な解決: スクリーンショットで確認された特定のテスト用請求書番号のみを「狙い撃ち」で削除
        target_invoices = [
            'INV-AG-1-04030925',
            'INV-20260406134552',
            'INV-AG-2-04091040',
        ...
        for inv_num in target_invoices:
            inv = db.query(models.Invoice).filter(models.Invoice.invoice_number == inv_num).first()
            if inv:
                results.append(f"  - {inv_num} を削除中...")
                inv_id = inv.id
                ...
                results.append(f"  - {inv_num} は既に見つかりません（スキップ）")

        # 2. その他、4/10以前の「テスト用シャドウデータ」の残りカスのみを掃除
        ...
        db.commit()
        results.append("✅ データベースの安全なクリーンアップが完了しました。")
        results.append("※ 特定のテストデータのみを削除しました。本物の受注には一切影響を与えていません。")
